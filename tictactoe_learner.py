...
class Player:
    ...
    def calculateNonFinalBoardScore(self, weight_vector, feature_vector):
        ...
        weight_vector = np.array(weight_vector).reshape(
            (len(weight_vector), 1))
        feature_vector = np.array(feature_vector).reshape(
            (len(feature_vector), 1))
    ...
    def chooseMove(self, board, playerSymbol1, playerSymbol2):
        ...
        legalMoveScores = [
            self.calculateNonFinalBoardScore(self.playerTargetFunctionWeightVector,
...
class PerformanceSystem:
    ...
    def generateGameHistory(self):
        ''' Returns Solution trace generated from pitting 2 players(agents) against each other'''

        gameHistory = []
        gameStatusFlag = True
        player1 = Player(
            self.playerSymbols[0], self.playersTargetFunctionWeightVectors[0])
        player2 = Player(
            self.playerSymbols[1], self.playersTargetFunctionWeightVectors[1])
        tempBoard = copy.deepcopy(self.board)
        while (gameStatusFlag):
            tempBoard = player1.chooseMove(
                tempBoard, player1.playerSymbol, player2.playerSymbol)
            gameHistory.append(tempBoard)
            gameStatusFlag = not self.isGameOver(
                tempBoard, player1.playerSymbol)
            if (gameStatusFlag == False):
                break
            tempBoard = player2.chooseRandomMove(
                tempBoard, player2.playerSymbol)
            gameHistory.append(tempBoard)
            gameStatusFlag = not self.isGameOver(
                tempBoard, player2.playerSymbol)
        return (gameHistory)
...
class Critic:
    ...
    def generateTrainingSamples(self, weight_vector, playerSymbol1, playerSymbol2):
        ...
        for i in range(len(self.gameHistory)-1):
            feature_vector = self.extractFeatures(
                self.gameHistory[i+1], playerSymbol1, playerSymbol2)
            trainingExamples.append(
                [feature_vector, self.calculateNonFinalBoardScore(weight_vector, feature_vector)])
    ...
    def boardDisplay(self, playerSymbol1, playerSymbol2, gameStatusCount):
        ...
        finalScore = self.calculateFinalBoardScore(
            self.gameHistory[-1], playerSymbol1, playerSymbol2)
        if (finalScore == 100):
...
class Generalizer:
    ...
    def lmsWeightUpdate(self, weight_vector, alpha=0.4):
        ...
        for trainingExample in self.trainingExamples:
            ...
            vHatBoardState = self.calculateNonFinalBoardScore(
                weight_vector, trainingExample[0])
            weight_vector = weight_vector + \
                (alpha * (vTrainBoardState - vHatBoardState)
                 * np.array(trainingExample[0]))
...
def train(numTrainingSamples=10):
    ...
    trainingGameCount = 0
    playerSymbols = ('X', 'O')
    playersTargetFunctionWeightVectors = [
        np.array([.5, .5, .5, .5, .5, .5, .5]), np.array([.5, .5, .5, .5, .5, .5, .5])]
    ...
    while (trainingGameCount < numTrainingSamples):
        ...
        performanceSystem = PerformanceSystem(
            initialBoardState, playersTargetFunctionWeightVectors, playerSymbols)
        ...
        gameStatusCount = critic.boardDisplay(
            playerSymbols[0], playerSymbols[1], gameStatusCount)
    ...
    while (ans == "y"):
        ...
        while (gameStatusFlag):
            ...
            boardState = computer.chooseMove(
                boardState, computer.playerSymbol, 'H')
            print('Computers\'s Turn:\n')
            computer.boardPrint(boardState)
            gameHistory.append(boardState)
            gameStatusFlag = not computer.isGameOver(
                boardState, computer.playerSymbol)
            if (gameStatusFlag == False):
                break
